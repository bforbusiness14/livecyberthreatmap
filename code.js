...
// Define colors for each attack pattern
const malwareColor = '#FF5733';
const phishingColor = '#8b41e0';
const exploitColor = '#dbde2a';

// Update the country data to simulate random attacks
// Example: Assign a random attack pattern to each country
arcData.forEach(item => {
    const randomIndex = Math.floor(Math.random() * 3); // Generate a random index (0, 1, or 2)
  
    switch (randomIndex) {
        case 0:
            item.attackType = 'Malware';
            item.options.strokeColor = malwareColor;
            
            break;
        case 1:
            item.attackType = 'Phishing';
            item.options.strokeColor = phishingColor;
            
            break;
        case 2:
            item.attackType = 'Exploit';
            item.options.strokeColor = exploitColor;
...
const mapLines = (map, data) => {
  ...
  map.svg.selectAll('.arch-container')
    .append('svg:path')
      ...
      .attr('stroke-width', () => {
        return data.options.strokeWidth * 2;
      })
      .attr('fill', 'none')      
  ...
  setTimeout(() => {
    for (let i = 0; i < arcData.length; i++) {
      ...
      setTimeout(() => {
        ...
        // Clear markers and lines after each iteration
        setTimeout(() => {
          clearLinesMarkersArrows(map);
          tempMarkers = [];
          tempLabels = [];
        ...
        // Construct new attack information using originMarker and destinationMarker
        const newAttackType = arcData[i].attackType; // Extracting attack type from arcData
        const newAttackInfo = `${arcData[i].attackType}:\n ${originMarker.name} -> ${destinationMarker.name}`;
        
        
        updateSidebar(newAttackInfo, newAttackType);
...
// Function to update the sidebar with new attack information
        function updateSidebar(attackInfo, attackType) {
            const attackList = document.getElementById('attack-list');
            const listItem = document.createElement('li');

//test
        // Create a span element for the legend icon
          const legendIcon = document.createElement('span');
          legendIcon.className = 'legend-icon ' + attackType.toLowerCase(); // Use attackType to set the class name
          listItem.appendChild(legendIcon); // Append the legend icon to the list item

        // Create a span element for the attack information
    const attackInfoSpan = document.createElement('span');
    attackInfoSpan.textContent = attackInfo;
    listItem.appendChild(attackInfoSpan); // Append the attack information to the list item

    attackList.insertBefore(listItem, attackList.firstChild);

            // Test - Update attack types
    // const attackTypeElement = document.createElement('span');
    // attackTypeElement.textContent = attackType;
    // attackTypes.appendChild(attackTypeElement);

            

            // Remove excess items if more than 6
            if (attackList.children.length > 6) {
                attackList.removeChild(attackList.lastChild);
                // attackTypes.removeChild(attackTypes.lastChild);
            }
        }
